...
function buildConstrainedMobilityPriorPieces({ subjectSpecies, subjectTeam, movingTeam, random }) {
  let pieces = new Map()

  // Place the subject piece
  const subjectResult = placeNextPiece({ pieces, species: subjectSpecies, team: subjectTeam, random })
  if (!subjectResult) { return null }
  pieces = subjectResult.pieces
  const subjectSquare = subjectResult.square

  // Place blockers near the subject from the full BLOCKER_SPECIES_POOL.
  // Prefer movingTeam pieces so that the un-blocking move belongs to the moving side.
  const blockerCount = 3 + Math.floor(random() * 4)
  const nearbySquares = shuffled(
    ALL_POSITIONS.filter(sq => {
      if (squareIsOccupied(pieces, sq)) { return false }
      const rankDist = Math.abs(Board.rankIndex(sq) - Board.rankIndex(subjectSquare))
      const fileDist = Math.abs(Board.fileIndex(sq) - Board.fileIndex(subjectSquare))
      return rankDist <= 3 && fileDist <= 3
    }),
    random
  )
  ...
  for (const sq of nearbySquares) {
    ...
    const species = validSpecies[Math.floor(random() * validSpecies.length)]
    const next = placeAtSquare({ pieces, species, team, square: sq })
    if (next) { pieces = next; placed++ }
  }

  // Ensure at least one movingTeam non-king piece exists (to be moved in the forward pass)
  const hasMovingPiece = Array.from(pieces.entries()).some(([, p]) => {
    return p.startsWith(movingTeam) && !p.endsWith(Board.KING)
  })
  if (!hasMovingPiece) {
    const fallbackSpecies = BLOCKER_SPECIES_POOL[Math.floor(random() * BLOCKER_SPECIES_POOL.length)]
    const fallbackResult = placeNextPiece({ pieces, species: fallbackSpecies, team: movingTeam, random })
    if (!fallbackResult) { return null }
    pieces = fallbackResult.pieces
  }

  return placeKingsIfAbsent(pieces, random)
}

function collectMobilityForwardExamples({ combinedPlan, unaryPlan, item, random, maxResults }) {
  const { subjectTeam } = unaryPlan
  const { movingTeam, evaluationPayloads } = combinedPlan
  const subjectSpecies = item.species
  if (!subjectSpecies) { return [] }

  const examples = []
  const evaluator = new ConditionEvaluatorV2()

  for (let attempt = 0; attempt < MAX_FORWARD_MOBILITY_ATTEMPTS && examples.length < maxResults; attempt++) {
    const priorPieces = buildConstrainedMobilityPriorPieces({ subjectSpecies, subjectTeam, movingTeam, random })
    if (!priorPieces) { continue }

    const priorLayout = buildLayoutFromPieces(priorPieces)
    const priorBoard = buildBoardFromLayout(priorLayout, null, movingTeam)

    const movingPieces = shuffled(
      Array.from(priorPieces.entries()).filter(([, p]) => p.startsWith(movingTeam) && !p.endsWith(Board.KING)),
      random
    )

    let foundForAttempt = false
    for (const [from] of movingPieces) {
      if (foundForAttempt || examples.length >= maxResults) { break }

      const destinations = shuffled([...ALL_POSITIONS].filter(to => to !== from), random)
      for (const to of destinations) {
        if (foundForAttempt) { break }

        let moveObject
        try {
          moveObject = Rules.getMoveObject(from, to, priorBoard)
        } catch {
          continue
        }
...
export function collectUnaryExamples({ combinedPlan, unaryPlan, item, random, maxResults = 3 }) {
  const { target, operator, comparator } = unaryPlan
  const isPBS = target === PRIOR_BOARD_TARGET
  const isPBSMobilityIncreasing = isPBS && operator === 'mobility' && isIncreasingComparator(comparator)
  const isGroupSubject = unaryPlan.subject === 'allied' || unaryPlan.subject === 'enemy'

  if (isPBSMobilityIncreasing && isGroupSubject && item.species) {
    return collectMobilityForwardExamples({ combinedPlan, unaryPlan, item, random, maxResults })
  }
