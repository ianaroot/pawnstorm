...
export function buildSeedFromPreset(combinedPlan, specialPreset, moveKind, random) {
  const requiredPieces = buildRequiredPieces(combinedPlan.requiredPositions, random)
  if (!requiredPieces) { return null }

  const recentMoveContext = (moveKind === MOVE_KIND_EN_PASSANT && specialPreset)
  ...
  if (relationalPlans.length === 0) {
    ...
    return { pieces: fixedPieces, reservedSquares, recentMoveContext, moveKind, geometryKey, relationalPositions: [] }
  }
  ...
  for (const plan of relationalPlans) {
    ...
    const fixedSubjectPlacement = SINGULAR_ACTORS.has(plan.subject)
      ? (placedActors.get(plan.subject)
          ?? (plan.subjectFilter === 'king' ? findExistingKingPlacement(plan.subjectTeam, currentPieces) : null))
      : (plan.subjectFilter === 'king' ? findExistingKingPlacement(plan.subjectTeam, currentPieces) : null)
    const fixedTargetPlacement = SINGULAR_ACTORS.has(plan.target)
    ...
    // same_piece has no geometric placement (it's a position+species match
    // between recentMoveContext and the captured piece). The seed doesn't
    // need to place anything for this plan; recentMoveContext on the prior
    // board carries the equivalence.
  ...
  return { pieces: currentPieces, reservedSquares, recentMoveContext, moveKind, geometryKey: geometryKeys.join(':'), relationalPositions }
}

// ===== buildSeed =====

export function buildSeed(combinedPlan, moveKind, random) {
  let specialPreset = null

  if (moveKind === MOVE_KIND_CASTLE) {
    const presets = shuffled(castlePresetsForTeam(combinedPlan.movingTeam), random)
    specialPreset = presets[0] ?? null
  } else if (moveKind === MOVE_KIND_PROMOTION) {
    const presets = shuffled(promotionPresetsForTeam(combinedPlan.movingTeam), random)
    specialPreset = presets[0] ?? null
  } else if (moveKind === MOVE_KIND_EN_PASSANT) {
    const presets = shuffled(enPassantPresetsForTeam(combinedPlan.movingTeam), random)
    specialPreset = presets[0] ?? null
  }

  if (moveKind !== MOVE_KIND_STANDARD && specialPreset === null) { return null }
  return buildSeedFromPreset(combinedPlan, specialPreset, moveKind, random)
}
